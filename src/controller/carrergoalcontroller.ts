...
export const createCareerGoal = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "Failed to create career goal", details: err });
  }
};
...
export const getCareerGoals = async (req: Request, res: Response) => {
  try {
    const goals = await CarrerData.find();
    res.status(200).json({ goals });
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch career goals", details: err });
  }
};
...
export const updateCareerGoal = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Failed to update career goal", details: err });
  }
};
